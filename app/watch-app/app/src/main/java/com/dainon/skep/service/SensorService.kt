...
import com.dainon.skep.data.SensorPayload
import com.dainon.skep.data.ServerClient
import com.dainon.skep.ui.MainActivity
...
class SensorService : Service(), SensorEventListener {
    ...
    companion object {
        ...
        const val CHANNEL_ID = "skep_sensor"
        const val CHANNEL_ALERT = "skep_alert"
        ...
        // 확인 버튼 액션
        const val ACTION_ACKNOWLEDGE = "com.dainon.skep.ACKNOWLEDGE"
        const val ACTION_MANUAL_EMERGENCY = "com.dainon.skep.MANUAL_EMERGENCY"
        const val ACTION_DISMISS = "com.dainon.skep.DISMISS"  // 오작동 종료 (학습 안 함)
        const val ACTION_LAUNCH_P2P = "com.dainon.skep.LAUNCH_P2P"  // P2P 화면 띄우기
        const val ACTION_P2P_RESPOND = "com.dainon.skep.P2P_RESPOND"    // ★ 수신자 "응답함"
        const val ACTION_P2P_CANT_HELP = "com.dainon.skep.P2P_CANT_HELP" // ★ 수신자 "도움불가"
    }
    ...
    // ──── GPS 위치 ────
    // 자기 GPS 실측 잡히기 전까지는 null. 폰 페어링 시 받은 좌표가 SharedPreferences("skep") 의
    // phoneLat/phoneLng 에 백업으로 들어가고, 자기 GPS 없을 때 currentLat()/currentLng() 가 그걸 사용.
    private var latitude: Double? = null
    private var longitude: Double? = null

    private fun currentLat(): Double? {
        if (latitude != null) return latitude
        val v = getSharedPreferences("skep", MODE_PRIVATE).getString("phoneLat", null) ?: return null
        return v.toDoubleOrNull()
    }
    private fun currentLng(): Double? {
        if (longitude != null) return longitude
        val v = getSharedPreferences("skep", MODE_PRIVATE).getString("phoneLng", null) ?: return null
        return v.toDoubleOrNull()
    }
    ...
    override fun onCreate() {
        ...
        val prefs = applicationContext.getSharedPreferences("skep", MODE_PRIVATE)
        WORKER_ID = prefs.getString("workerId", "W-001") ?: "W-001"

        // 서버 URL + FieldWorker 토큰 로드
        val savedUrl = prefs.getString("serverUrl", "") ?: ""
        if (savedUrl.isNotBlank()) ServerClient.updateUrl(savedUrl)
        ServerClient.updateFieldToken(prefs.getString("fieldToken", "") ?: "")
        ...
        // 폰에서 worker_id 주입 + ACK 수신 (Wearable Data Layer)
        startPhoneListener()
        ...
        scope.launch {
            try {
                ...
                if (registry.isNotBlank()) {
                    applicationContext.getSharedPreferences("skep", MODE_PRIVATE).edit()
    ...
    /** 폰 → 워치 메시지 수신 리스너 (worker_id 주입 + ACK/dismiss) */
    ...
    /**
     * 폰이 보낸 작업자 신원 저장 (CONTRACT: 워치는 폰에서 worker_id 수신).
     * payload(JSON): {"worker_id": "...", "token": "..."(옵션)}
     */
    private fun handleWorkerIdFromPhone(data: ByteArray) {
        try {
            val json = String(data, Charsets.UTF_8)
            val map = com.google.gson.Gson().fromJson(json, Map::class.java)
            val wid = (map["worker_id"] as? String)?.trim().orEmpty()
            val token = (map["token"] as? String)?.trim().orEmpty()
            if (wid.isNotBlank()) {
                WORKER_ID = wid
                val prefs = applicationContext.getSharedPreferences("skep", MODE_PRIVATE)
                prefs.edit().putString("workerId", wid).apply()
                if (token.isNotBlank()) {
                    prefs.edit().putString("fieldToken", token).apply()
                    ServerClient.updateFieldToken(token)
                }
                // 새 ID로 BLE 광고 재시작
                BleAlertService.startNormalAdvertise(this, WORKER_ID)
                Log.d(TAG, "📱 worker_id from phone: $wid (token=${token.isNotBlank()})")
            }
        } catch (e: Exception) {
            Log.w(TAG, "worker_id parse failed: ${e.message}")
        }
    }
    ...
    private fun evaluateState() {
        ...
        when (currentState) {
            ...
            WorkerState.EMERGENCY -> {
                ...
                // 1회만: 알람 + BLE 경보 + AckAlertActivity + 안전알림 발신
                if (!isEmergencyAlarmActive) {
                    ...
                    // ★ 폰으로 안전알림 1회 발신 (폰이 서버로 중계: /api/field/safety-alert)
                    sendSafetyAlert("emergency")
                }

                // 폰으로의 실시간 바이탈은 broadcastStatus()의 sendStatus가 매 틱 전송
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_P2P_RESPOND, ACTION_P2P_CANT_HELP -> {
                ...
                sendBroadcast(Intent("com.dainon.skep.CLOSE_P2P").apply {
                    setPackage("com.dainon.skep")
                })
            }
        }
        return START_STICKY
    ...
    /**
     * 폰으로 안전알림 발신 (Wearable Data Layer → 폰이 서버로 중계).
     * 옵션: 워치가 직접 서버(/api/field/safety-alert)로도 보냄 — 폰 미연결 대비.
     */
    private fun sendSafetyAlert(kind: String) {
        val spo2Val = if (spo2 > 0) spo2 else 98
        val lat = currentLat()
        val lng = currentLng()
    ...
    private fun launchAlertActivity(state: String, message: String) {
        val ackIntent = Intent(this, com.dainon.skep.ui.AckAlertActivity::class.java).apply {
    ...
    private fun launchP2pAlert(workerName: String, workerId: String, distance: Double, zone: String) {
        val alertIntent = Intent(this, com.dainon.skep.ui.P2pAlertActivity::class.java).apply {
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
    ...
    private fun broadcastStatus() {
        ...
        sendBroadcast(Intent("com.dainon.skep.SENSOR_UPDATE").apply {
            setPackage("com.dainon.skep")
    ...
    private fun broadcastAutoEndWork() {
        sendBroadcast(Intent("com.dainon.skep.AUTO_END_WORK").apply {
            setPackage("com.dainon.skep")
        })
        // 서버에 offline 1회 전송
        scope.launch {
    ...
    private fun broadcastWatchBackOn() {
        sendBroadcast(Intent("com.dainon.skep.WATCH_BACK_ON").apply {
            setPackage("com.dainon.skep")
        })
    }
