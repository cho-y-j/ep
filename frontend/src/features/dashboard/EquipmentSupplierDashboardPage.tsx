...
export default function EquipmentSupplierDashboardPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    api.get<Summary>('/api/dashboards/equipment-supplier/summary')
  ...
  return (
    <AppShell breadcrumb={[{ label: '대시보드' }]}>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold">{company?.name ?? '장비공급사'} 대시보드</h1>
          <p className="text-sm text-slate-500 mt-1">
            {user?.name}님, 내 장비 운영과 참여 현장을 확인하세요.
          </p>
        </div>

        <SupplierTodayTasksWidget role="EQUIPMENT_SUPPLIER" companyId={user?.company_id} expiringDocsCount={c.documents_expiring30d ?? 0} />
          ...
          <>
            {isMaster ? (
              <section className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <StatCard label="내 장비" value={c.my_equipment ?? 0} href="/equipment" tone="blue" />
                <StatCard label="배치 중 장비" value={c.equipment_assigned ?? 0} tone="emerald" />
            ...
            <MiniBarChart
              title="장비 배치 현황"
              data={[
                { label: '배치 중', value: c.equipment_assigned ?? 0, tone: 'emerald' },
                { label: '대기', value: Math.max(0, (c.my_equipment ?? 0) - (c.equipment_assigned ?? 0)), tone: 'slate' },
              ]}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <WorkPlanListWidget title="내 장비가 포함된 작업계획서" items={summary?.upcoming_work_plans ?? []} showBpName />
            </div>

            <DocumentRiskWidget id="doc-risks" title="장비/조종원 서류 위험" items={summary?.document_risks ?? []} />
          </>
        )}
      </div>
    </AppShell>
