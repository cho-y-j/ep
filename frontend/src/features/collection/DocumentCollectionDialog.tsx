...
export default function DocumentCollectionDialog({ ownerType, ownerId, ownerLabel, onClose }: Props) {
  ...
  async function reload() {
    ...
    try {
      const [t, r, s] = await Promise.all([
        ...
        api.get<CollectionSummary[]>('/api/document-collections', { params: { ownerType, ownerId } }),
  ...
  async function create() {
    ...
    try {
      await api.post('/api/document-collections', {
        ...
        // 상세화면에서 만드는 링크는 대상 1건 — 다중 대상은 서류 수집 요청 페이지에서.
        targets: [{
  ...
  async function sendLinkSms(req: CollectionSummary) {
    setBusy(true); setError(null);
    try {
                ...
                <div key={req.id} className="rounded-lg border border-slate-200 p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500">
                      {req.target_count > 1 && `대상 ${req.target_count}건 · `}{req.uploaded_count}/{req.item_count} 업로드
                    </span>
                  </div>
