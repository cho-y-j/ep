...
export default function DispatchedEquipmentBpPage() {
  ...
  const categoryOptions = useMemo(() => {
    ...
    rows.forEach((r) => { if (r.equipment_category) s.add(r.equipment_category); });
    return [...s].map((c) => ({ value: c, label: c }));
  }, [rows]);
  const qLower = q.trim().toLowerCase();
  const filtered = useMemo(() => rows.filter((r) => {
    if (supplierFilter && String(r.supplier_company_id) !== supplierFilter) return false;
    if (categoryFilter && r.equipment_category !== categoryFilter) return false;
    if (qLower) {
      const hay = `${r.equipment_label ?? ''} ${r.supplier_company_name ?? ''} ${r.equipment_category ?? ''}`.toLowerCase();
      if (!hay.includes(qLower)) return false;
    }
    return true;
  }), [rows, supplierFilter, categoryFilter, qLower]);
  const activeFilterCount = [q, supplierFilter, categoryFilter].filter(Boolean).length;
  const resetFilters = () => { setQ(''); setSupplierFilter(''); setCategoryFilter(''); };
  ...
  return (
    <AppShell breadcrumb={[{ label: '투입 장비' }]}>
      <div className="mx-auto max-w-7xl space-y-4">
        <PageHeader
          title="투입 장비"
          subtitle="견적 응답으로 받은 차량 — 작업계획서에 투입하거나 투입 중인 장비를 모아봅니다."
        />

        <FilterBar
          search={{ value: q, onChange: setQ, placeholder: '장비명 / 공급사 / 카테고리 검색' }}
        ...
        >
          ...
          <FilterSelect value={categoryFilter} onChange={setCategoryFilter} placeholder="카테고리 전체" options={categoryOptions} />
        </FilterBar>
