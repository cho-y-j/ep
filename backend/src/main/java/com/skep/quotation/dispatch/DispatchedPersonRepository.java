...
public interface DispatchedPersonRepository extends JpaRepository<DispatchedPerson, Long> {
    ...
    @Query("select distinct d.supplierCompanyId from DispatchedPerson d")
    List<Long> findDistinctSupplierCompanyIds();

    /** 월마감 알림 대상: 자기 귀속(sub_supplier) distinct 공급사 id (null 제외). */
    @Query("select distinct d.subSupplierCompanyId from DispatchedPerson d where d.subSupplierCompanyId is not null")
